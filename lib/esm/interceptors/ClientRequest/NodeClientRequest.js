...
import { normalizeClientRequestWriteArgs, } from './utils/normalizeClientRequestWriteArgs';
...
export class NodeClientRequest extends ClientRequest {
    ...
    write(...args) {
        ...
        until(async () => {
            ...
            this.once('response-internal', async (response) => {
    ...
    respondWith(mockedResponse) {
        ...
        if (headers) {
            ...
            for (const [headerName, headerValue] of Object.entries(headers)) {
                this.response.rawHeaders.push(headerName, ...(Array.isArray(headerValue) ? headerValue : [headerValue]));
    ...
    getRequestBody(chunk) {
        const writtenRequestBody = bodyBufferToString(Buffer.concat(this.requestBody));
